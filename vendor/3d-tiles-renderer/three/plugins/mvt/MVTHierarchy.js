...
class MVTTile {
	...
	constructor() {
		...
		// ref count of consumers that want this tile's content fetched without displaying it, used
		// to start the download before the tile is needed on screen
		this.prefetch = 0;
...
export class MVTHierarchy extends EventDispatcher {
	...
	update() {
		...
		function updateState( tile ) {
			...
			if ( targeted || forcedTarget ) {
				...
				if ( tile.hideTimer === TIMER_DURATION ) {
					...
					// state before any async callbacks can observe it. Content held for a prefetch
					// is kept, and released below once nothing is asking for it.
			...
			// content that is only prefetched is dropped as soon as nothing wants it, since the
			// hide timer above never runs for a tile that was never shown
			if ( tile.prefetch === 0 && ! targeted && ! forcedTarget && tile.showTimer === 0 && tile.loadingState !== UNLOADED ) {
			...
			// Kick off load once the show timer commits to this tile, or as soon as it is prefetched
		...
		function updateVisibility( tile, force = false, siblingsReady = true ) {
			...
			// showTimer > 0 keeps tiles with in-flight loads from being pruned mid-hysteresis, and a
			// prefetched tile is kept so the content it is holding is not thrown away
	...
	// Marks a tile's content as wanted without displaying it, so the download can be started before
	// the tile is needed on screen. Ref counted in the same way as the target state.
	setPrefetchState( x, y, l, prefetch ) {
