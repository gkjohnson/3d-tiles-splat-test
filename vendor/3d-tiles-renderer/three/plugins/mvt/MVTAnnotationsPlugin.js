...
export class MVTAnnotationsPlugin {
	...
	constructor( options = {} ) {
		...
		// the tiles currently holding a prefetch, so the ref counts cannot be added twice for the
		// same tile or released without having been taken
		this._prefetchedTiles = new Set();
	...
	dispose() {
		...
		// visible tiles are the ones currently marked in the hierarchy, and every loaded tile is
		// holding a prefetch
	...
	processTileModel( scene, tile ) {
		...
		// start fetching the vector tiles covering this geometry now rather than waiting for it to
		// become visible, so the two downloads overlap
	...
	_prefetchVectorTile( tile, state ) {
		...
		const { overlay, _prefetchedTiles } = this;

		if ( state ) {

			// tiles can finish parsing before the overlay has initialized, and are picked up by the
			// late initialization pass once it has
			if ( ! overlay.isReady || _prefetchedTiles.has( tile ) ) {

				return;

			}

			_prefetchedTiles.add( tile );

		} else if ( ! _prefetchedTiles.delete( tile ) ) {
