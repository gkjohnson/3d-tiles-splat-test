...
export class ImageOverlayPlugin {
	...
	async _initTileSceneOverlayInfo( scene, tile, overlay = this.overlays ) {
		...
		// the tiled image set the don't allocate a texture for it. The tile's depth is passed so an
		// overlay can limit itself to a range of levels.
		if ( heightInRange && overlay.hasContent( range, tile.internal.depth ) ) {
...
export class GeoJSONOverlay extends ImageOverlay {
	...
	constructor( options = {} ) {
		...
		const { minLevel = - Infinity, maxLevel = Infinity } = options;

		/**
		 * The tile depths the overlay draws at. Outside this range the overlay reports no content,
		 * so it is left off the tiles entirely rather than drawn and faded.
		 * @type {number}
		 * @default -Infinity
		 */
	...
	hasContent( range, level = null ) {
		...
		if ( level !== null && ( level < this.minLevel || level > this.maxLevel ) ) {
